...
def fit(model, optimizer_ADAM, optimizer_LBFGS, epoch_ADAM, training_set_class, validation_set_clsss=None, verbose=False, training_ic=False):
        ...
        if len(training_boundary) != 0 and len(training_initial_internal) != 0:
            for step, ((x_coll_train_, u_coll_train_), (x_b_train_, u_b_train_), (x_u_train_, u_train_)) in enumerate(
                ...
                def closure():
                    ...
                    # 在closure内部记录（LBFGS每次迭代都会调用）
                    inner_iteration[0] += 1
                    if inner_iteration[0] % 10 == 0:  # 每10次LBFGS迭代记录一次
                        training_history['epochs'].append(iteration * 1000 + inner_iteration[0])  # 用组合索引
                        training_history['total_loss'].append(float(loss_f.detach().cpu()))
                ...
                # 也记录最终loss（每个batch结束）
                if len(training_history['epochs']) == 0 or training_history['epochs'][-1] != iteration:
                    training_history['epochs'].append(iteration)
                    training_history['total_loss'].append(float(train_losses[0].detach().cpu()))
                    training_history['time'].append(time.time() - start_time)
                iteration += 1
            ...
            for step, ((x_coll_train_, u_coll_train_), (x_u_train_, u_train_)) in enumerate(zip(training_coll, training_initial_internal)):
                ...
                def closure():
                    ...
                    # 在closure内部记录
                    inner_iteration[0] += 1
                    if inner_iteration[0] % 10 == 0:
                        training_history['epochs'].append(iteration * 1000 + inner_iteration[0])
                        training_history['total_loss'].append(float(loss_f.detach().cpu()))
                        training_history['time'].append(time.time() - start_time)
    ...
    try:
        ...
        for frame in inspect.stack():
            if 'folder_path' in frame.frame.f_locals:
                save_dir = frame.frame.f_locals['folder_path']
                break
            elif 'folder_path' in frame.frame.f_globals:
                save_dir = frame.frame.f_globals['folder_path']
                break
        ...
        # 备用：保存到当前目录
        try:
            with open('training_history_backup.json', 'w') as f:
                json.dump(training_history, f, indent=2)
            print(f"Training history saved to training_history_backup.json (fallback)")
        except:
            pass
